...
- [6. 🤔 为何需要确保服务是单例？](#6--为何需要确保服务是单例)
- [7. 🤔 如何实现单例服务？](#7--如何实现单例服务)
- [8. 🤔 为何不输出 vitepress 服务启动的真实进度百分比，而是展示一个启动计时器？](#8--为何不输出-vitepress-服务启动的真实进度百分比而是展示一个启动计时器)
- [9. 🤔 vitepress 服务启动的超时时间是？](#9--vitepress-服务启动的超时时间是)
- [10. 🤔 TNotes 笔记监听服务的启动流程是？](#10--tnotes-笔记监听服务的启动流程是)
  - [10.1. 步骤 1：初始化笔记索引缓存](#101-步骤-1初始化笔记索引缓存)
  - [10.2. 步骤 2：启动文件监听服务](#102-步骤-2启动文件监听服务)
...
## 8. 🤔 为何不输出 vitepress 服务启动的真实进度百分比，而是展示一个启动计时器？
...
# 🚀 [23:08:48.021] [dev] 启动知识库开发服务
# ℹ️  [23:08:48.024] [dev] 扫描到 40 篇笔记
# ✅ [23:08:49.495] [dev] VitePress 服务（v1.6.4）已就绪，耗时：1261 ms
# ✅ [23:08:49.505] [dev] 文件监听服务已就绪，耗时：10 ms
# ℹ️  [23:08:49.505] [dev] 本地开发服务地址：http://localhost:9379/TNotes.introduction/
# ✨ [23:08:49.505] [dev] 命令执行耗时：1484 ms
```
...
`DevCommand` 直接依赖 `NoteIndexCache` 和 `FileWatcherService`，不通过中间层，各模块职责清晰：
